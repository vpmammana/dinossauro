...
- mantenha SETA PARA ESQUERDA/DIREITA: virar;
...
GENERATOR_VERSION = "4.1-teclado-corrigido"
FPS = 24
FRAME_START = 1
FRAME_END = 24
LOOP_KEY = 25
...
def ensure_node_tree(id_block):
    """
    Retorna a árvore de nós sem tocar em ``use_nodes`` quando ela já existe.

    Em versões antigas do Blender, materiais e mundos novos podem precisar
    que ``use_nodes`` seja ativado. A atribuição fica restrita a esse caminho
    de compatibilidade e o aviso de depreciação é silenciado localmente.
    """
...
class WM_OT_parasaurolophus_keyboard_controller(bpy.types.Operator):
    ...
    bl_description = (
        "Setas para cima/baixo deslocam; esquerda/direita viram"
    )
    bl_options = {"REGISTER"}
    ...
    def _reset_position(self):
        ...
        self._master.rotation_euler.z = 0.0
    ...
    def _tick(self, context):
        ...
        turn_direction = int(self._left_pressed) - int(self._right_pressed)
        walk_speed = float(self._scene.parasauro_walk_speed)
        ...
        if turn_direction != 0:
            speed_ratio = min(
                1.0,
                abs(self._velocity) / max(walk_speed, 0.001),
            )
            steering_factor = 0.35 + 0.65 * speed_ratio
            turn_radians = float(self._scene.parasauro_turn_speed)
            new_heading = self._master.rotation_euler.z + (
                turn_direction
                * turn_radians
                * steering_factor
                * delta_time
        ...
        if (
            ...
            self._follow_rig.rotation_euler.z = (
                self._master.rotation_euler.z
            )

        position = self._master.location
        limit_text = " | LIMITE DO TERRENO" if reached_limit else ""
        self._set_header(
            "Parasaurolophus: ↑ frente  ↓ ré  ←/→ virar  "
            "HOME centralizar  "
    ...
    def invoke(self, context, event):
        ...
        self._set_header(
            "Parasaurolophus: ↑/↓ deslocam | ←/→ viram | "
            "HOME centraliza | ESC encerra"
        )
        self.report(
            {"INFO"},
            "Controle ativo: ↑/↓ deslocam e ←/→ viram.",
        )
    ...
    def modal(self, context, event):
        ...
        # As setas laterais são consumidas pelo operador. Isso evita que a
        # Timeline receba RIGHT_ARROW/LEFT_ARROW e tente pular keyframes.
        ...
        if event.type == "TIMER":
            # Algumas versões do Blender não expõem Event.timer. Quando o
            # atributo existe, filtramos pelo timer deste operador; quando não
            # existe, o próprio tipo TIMER é suficiente para atualizar.
...
class WM_OT_parasaurolophus_reset_position(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if controller is not None:
            ...
            controller._left_pressed = False
            controller._right_pressed = False
...
def register_keyboard_runtime():
    ...
    bpy.types.Scene.parasauro_turn_speed = FloatProperty(
        name="Velocidade de giro",
        description="Velocidade de rotação com as setas esquerda/direita",
        default=math.radians(80.0),
        min=math.radians(10.0),
        max=math.radians(180.0),
        soft_max=math.radians(120.0),
        subtype="ANGLE",
    )
    ...
    current_scene.parasauro_turn_speed = math.radians(80.0)
...
print("Use ↑/↓ para deslocar, ←/→ para virar, HOME para o centro e ESC para sair.")
print("=" * 72)
