"""
Carro estilo Fusca jovial — versão 4.7.5 parametrizada
======================================================

Correções desta versão:
- os recortes laterais usam exatamente o contorno do vidro, sem ampliação;
- o cutter entra apenas uma pequena distância na parede lateral;
- a maior parte do cutter fica para fora da carroceria;
- isso impede a face interna do cutter de alcançar o teto/capô curvado;
- a Body_Shell é subdividida longitudinalmente e transversalmente antes
  dos booleans, reduzindo o aspecto serrilhado da interseção;
- o vidro traseiro continua removido, permanecendo somente o vão;
- preserva o para-brisa por contorno direto e a limpeza idempotente.
"""
...
GENERATOR_VERSION = "4.7.5-fusca-jovial-side-window-skin-cut"
GROUND_SIZE = 24.0
...
# A borda superior entra na cabine. O script inverte o sinal no lado direito.
SIDE_WINDOW_FRONT_INWARD_TILT_DEG = 12.0

# Cutter raso e deslocado para fora:
# somente INWARD_PENETRATION entra na carroceria.
SIDE_WINDOW_FRONT_CUTTER_DEPTH = 0.42
SIDE_WINDOW_FRONT_INWARD_PENETRATION = 0.15

# 1.0 = o vão usa exatamente o limite do vidro, sem ampliar para o capô.
SIDE_WINDOW_FRONT_OPENING_SCALE_X = 1.000
SIDE_WINDOW_FRONT_OPENING_SCALE_Z = 1.000
SIDE_WINDOW_FRONT_GLASS_OUTSET = 0.003
...
SIDE_WINDOW_REAR_INWARD_TILT_DEG = 12.0
SIDE_WINDOW_REAR_CUTTER_DEPTH = 0.40
SIDE_WINDOW_REAR_INWARD_PENETRATION = 0.14
SIDE_WINDOW_REAR_OPENING_SCALE_X = 1.000
SIDE_WINDOW_REAR_OPENING_SCALE_Z = 1.000
SIDE_WINDOW_REAR_GLASS_OUTSET = 0.003
...
def validate_parameters():
    positive = {
        ...
        "SIDE_WINDOW_FRONT_LENGTH": SIDE_WINDOW_FRONT_LENGTH,
        "SIDE_WINDOW_FRONT_HEIGHT": SIDE_WINDOW_FRONT_HEIGHT,
        "SIDE_WINDOW_FRONT_CUTTER_DEPTH": SIDE_WINDOW_FRONT_CUTTER_DEPTH,
        "SIDE_WINDOW_FRONT_INWARD_PENETRATION": SIDE_WINDOW_FRONT_INWARD_PENETRATION,
        "SIDE_WINDOW_FRONT_OPENING_SCALE_X": SIDE_WINDOW_FRONT_OPENING_SCALE_X,
        "SIDE_WINDOW_FRONT_OPENING_SCALE_Z": SIDE_WINDOW_FRONT_OPENING_SCALE_Z,
        "SIDE_WINDOW_REAR_LENGTH": SIDE_WINDOW_REAR_LENGTH,
        "SIDE_WINDOW_REAR_HEIGHT": SIDE_WINDOW_REAR_HEIGHT,
        "SIDE_WINDOW_REAR_CUTTER_DEPTH": SIDE_WINDOW_REAR_CUTTER_DEPTH,
        "SIDE_WINDOW_REAR_INWARD_PENETRATION": SIDE_WINDOW_REAR_INWARD_PENETRATION,
        "SIDE_WINDOW_REAR_OPENING_SCALE_X": SIDE_WINDOW_REAR_OPENING_SCALE_X,
        "SIDE_WINDOW_REAR_OPENING_SCALE_Z": SIDE_WINDOW_REAR_OPENING_SCALE_Z,
    ...
    if SIDE_WINDOW_FRONT_INWARD_PENETRATION >= SIDE_WINDOW_FRONT_CUTTER_DEPTH:
        raise ValueError(
            "SIDE_WINDOW_FRONT_INWARD_PENETRATION deve ser menor "
            "que SIDE_WINDOW_FRONT_CUTTER_DEPTH."
        )

    if SIDE_WINDOW_REAR_INWARD_PENETRATION >= SIDE_WINDOW_REAR_CUTTER_DEPTH:
        raise ValueError(
            "SIDE_WINDOW_REAR_INWARD_PENETRATION deve ser menor "
            "que SIDE_WINDOW_REAR_CUTTER_DEPTH."
        )
...
def side_window_profile_points(length, height):
    """Perfil trapezoidal no plano local XZ."""
    half_length = length * 0.5
    half_height = height * 0.5
    return [
        (-half_length, -half_height),
        ( half_length * 0.78, -half_height * 0.92),
        ( half_length,  half_height * 0.54),
        (-half_length * 0.65, half_height),
    ]


def side_window_rotation(side_sign, inward_tilt_deg):
    """
    left (+Y): +ângulo move o topo para -Y;
    right (-Y): -ângulo move o topo para +Y.
    """
    return (
        math.radians(float(side_sign) * inward_tilt_deg),
        0.0,
        0.0,
    )


def side_window_outward_normal(side_sign, inward_tilt_deg):
    rotation = side_window_rotation(side_sign, inward_tilt_deg)
    rotation_matrix = Euler(rotation, "XYZ").to_matrix()
    return (
        rotation_matrix
        @ Vector((0.0, float(side_sign), 0.0))
    ).normalized()


def create_side_window_surface(
    name,
    center,
    length,
    height,
    ...
    """Cria uma única superfície de vidro, sem lâminas transparentes duplas."""
    profile = side_window_profile_points(length, height)
    vertices = [(x, 0.0, z) for x, z in profile]
    faces = (
        [(0, 1, 2, 3)]
        if side_sign > 0
        else [(3, 2, 1, 0)]
    ...
    rotation = side_window_rotation(side_sign, inward_tilt_deg)
    outward = side_window_outward_normal(
        side_sign,
        inward_tilt_deg,
    )
    final_center = Vector(center) + outward * glass_outset
    ...
    obj = create_mesh_object(
        ...
        material=material,
        parent=parent,
        local_location=final_center,
        smooth=False,
        rotation=rotation,
    )
    obj["single_surface_glass"] = True
    obj["inward_tilt_deg"] = inward_tilt_deg
    return obj
...
def create_side_window_cutter(
    ...
    """
    Prisma raso para cortar somente a pele lateral.

    A face interna entra apenas ``inward_penetration`` na carroceria.
    O restante do volume fica do lado de fora, evitando atingir teto/capô.
    """
    if inward_penetration <= 0.0 or inward_penetration >= depth:
        raise ValueError(
            "inward_penetration deve ser maior que zero e menor que depth."
        )

    profile = side_window_profile_points(
        length * opening_scale_x,
        height * opening_scale_z,
    )
    ...
    center_shift = half_depth - inward_penetration
    cutter_center = Vector(center) + outward * center_shift

    vertices = (
        [(x, -half_depth, z) for x, z in profile]
        + [(x, half_depth, z) for x, z in profile]
    )
    faces = [
        (0, 1, 2, 3),
...
def create_side_window_with_opening(
    body_shell,
    name,
    cutter_name,
    center,
    ...
    """Abre primeiro o vão real e só depois cria o vidro lateral."""
    cutter = create_side_window_cutter(
        name=cutter_name,
        center=center,
        length=length,
        height=height,
        depth=cutter_depth,
        inward_penetration=inward_penetration,
        side_sign=side_sign,
        inward_tilt_deg=inward_tilt_deg,
        opening_scale_x=opening_scale_x,
        opening_scale_z=opening_scale_z,
        collection=collection,
        parent=parent,
    )
    ...
    if not apply_boolean_difference(
        ...
        raise RuntimeError(
            f"Não foi possível abrir o recorte real de {name} na Body_Shell."
        )

    return create_side_window_surface(
        name=name,
        center=center,
        length=length,
        height=height,
...
def build_cute_beetle(scene):
    ...
    # ----------------------------------------------------------------------
    side_window_l_front = create_side_window_with_opening(
        body_shell=body_shell,
        name="Side_Window_L_Front",
        cutter_name="CUT_Side_Window_L_Front",
        center=(SIDE_WINDOW_FRONT_X, SIDE_WINDOW_FRONT_Y, SIDE_WINDOW_FRONT_Z),
        length=SIDE_WINDOW_FRONT_LENGTH,
        height=SIDE_WINDOW_FRONT_HEIGHT,
        side_sign=1.0,
        inward_tilt_deg=SIDE_WINDOW_FRONT_INWARD_TILT_DEG,
        cutter_depth=SIDE_WINDOW_FRONT_CUTTER_DEPTH,
        inward_penetration=SIDE_WINDOW_FRONT_INWARD_PENETRATION,
        opening_scale_x=SIDE_WINDOW_FRONT_OPENING_SCALE_X,
        opening_scale_z=SIDE_WINDOW_FRONT_OPENING_SCALE_Z,
        glass_outset=SIDE_WINDOW_FRONT_GLASS_OUTSET,
        collection=model_collection,
        glass_material=materials["glass"],
        parent=root,
    )
    side_window_r_front = create_side_window_with_opening(
        body_shell=body_shell,
        name="Side_Window_R_Front",
        cutter_name="CUT_Side_Window_R_Front",
        center=(SIDE_WINDOW_FRONT_X, -SIDE_WINDOW_FRONT_Y, SIDE_WINDOW_FRONT_Z),
        length=SIDE_WINDOW_FRONT_LENGTH,
        height=SIDE_WINDOW_FRONT_HEIGHT,
        side_sign=-1.0,
    ...
    side_window_l_rear = create_side_window_with_opening(
        body_shell=body_shell,
        name="Side_Window_L_Rear",
        cutter_name="CUT_Side_Window_L_Rear",
        center=(SIDE_WINDOW_REAR_X, SIDE_WINDOW_REAR_Y, SIDE_WINDOW_REAR_Z),
        length=SIDE_WINDOW_REAR_LENGTH,
        height=SIDE_WINDOW_REAR_HEIGHT,
        side_sign=1.0,
        inward_tilt_deg=SIDE_WINDOW_REAR_INWARD_TILT_DEG,
        cutter_depth=SIDE_WINDOW_REAR_CUTTER_DEPTH,
        inward_penetration=SIDE_WINDOW_REAR_INWARD_PENETRATION,
        opening_scale_x=SIDE_WINDOW_REAR_OPENING_SCALE_X,
        opening_scale_z=SIDE_WINDOW_REAR_OPENING_SCALE_Z,
        glass_outset=SIDE_WINDOW_REAR_GLASS_OUTSET,
        collection=model_collection,
        glass_material=materials["glass"],
        parent=root,
    )
    side_window_r_rear = create_side_window_with_opening(
        body_shell=body_shell,
        name="Side_Window_R_Rear",
        cutter_name="CUT_Side_Window_R_Rear",
        center=(SIDE_WINDOW_REAR_X, -SIDE_WINDOW_REAR_Y, SIDE_WINDOW_REAR_Z),
        length=SIDE_WINDOW_REAR_LENGTH,
        height=SIDE_WINDOW_REAR_HEIGHT,
        side_sign=-1.0,
