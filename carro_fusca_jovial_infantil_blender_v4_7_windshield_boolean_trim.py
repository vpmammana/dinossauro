"""
Carro estilo Fusca jovial — versão 4.7 parametrizada
====================================================

Correção principal desta versão:
- remove a superfície de para-brisa criada por Shrinkwrap, que podia ficar
  amassada ou cair sobre o capô;
- mantém somente o objeto Windshield_Cutter_Base como vidro visível;
- o próprio Windshield_Cutter_Base é aparado por uma operação booleana
  INTERSECT contra uma cópia da Body_Shell;
- o vidro resultante segue o contorno real da carroceria no ponto do corte;
- uma cópia ligeiramente ampliada desse vidro abre o vão na Body_Shell;
- limpeza de vértices duplicados, recálculo de normais e bevel pequeno
  removem rebarbas e suavizam as bordas;
...
GENERATOR_VERSION = "4.7-fusca-jovial-windshield-boolean-trim"
GROUND_SIZE = 24.0
...
# ---------------------------------------------------------------------------
# WINDSHIELD — um único objeto aparado pela Body_Shell
# ---------------------------------------------------------------------------
# O objeto final visível chama-se Windshield_Cutter_Base.
# Ele nasce como um prisma trapezoidal e depois é recortado por INTERSECT
# contra a Body_Shell. Não existe mais grade separada nem Shrinkwrap.
WINDSHIELD_X = 0.47
WINDSHIELD_Y = 0.00
WINDSHIELD_Z = 1.30

WINDSHIELD_THICKNESS = 0.08
...
# Folga do vão aberto na Body_Shell ao redor do vidro já aparado.
# X amplia principalmente a espessura do cutter; Y e Z criam uma pequena junta.
WINDSHIELD_OPENING_SCALE_X = 1.40
WINDSHIELD_OPENING_SCALE_Y = 1.025
WINDSHIELD_OPENING_SCALE_Z = 1.025

# Acabamento das bordas depois da interseção booleana.
WINDSHIELD_EDGE_BEVEL = 0.006
...
def apply_boolean_intersection(target_obj, other_obj, remove_other=True):
    apply_object_transforms(target_obj, location=False, rotation=True, scale=True)
    apply_object_transforms(other_obj, location=False, rotation=True, scale=True)

    modifier = target_obj.modifiers.new(
        name=f"BOOL_INTERSECT_{other_obj.name}",
        type="BOOLEAN",
    )
...
def build_cute_beetle(scene):
    ...
    # ----------------------------------------------------------------------
    # WINDSHIELD: um único prisma aparado pela Body_Shell
    # ----------------------------------------------------------------------
    # Aplica a escala global da Body_Shell antes das operações booleanas.
    ...
    # Cópia temporária da carroceria ainda sem o vão do para-brisa.
    # Ela serve exclusivamente para aparar o vidro por INTERSECT.
    windshield_trim_proxy = duplicate_mesh_object(
        body_shell,
        "TMP_BodyShell_Windshield_Trim",
        model_collection,
        parent=None,
    )

    # Este é o único objeto de vidro frontal que permanecerá na cena.
    windshield = create_windshield_prism(
    ...
    # Recorta o próprio vidro pelo volume original da Body_Shell.
    # O resultado mantém apenas a parte que realmente coincide com a carroceria.
    if not apply_boolean_intersection(
        windshield,
        windshield_trim_proxy,
        remove_other=True,
    ):
        raise RuntimeError(
            "Não foi possível aparar Windshield_Cutter_Base pela Body_Shell."
        )

    if len(windshield.data.polygons) == 0:
        raise RuntimeError(
            "Windshield_Cutter_Base ficou vazio. Ajuste posição, rotação ou espessura."
        )
    ...
    # Abre o vão usando uma cópia levemente ampliada do vidro já aparado.
    # Assim, a abertura repete o mesmo contorno, mas deixa uma pequena junta.
    windshield_opening_cutter = duplicate_mesh_object(
        windshield,
        "CUT_Windshield_Opening",
        model_collection,
        parent=None,
    )
    windshield_opening_cutter.scale = (
        WINDSHIELD_OPENING_SCALE_X,
        WINDSHIELD_OPENING_SCALE_Y,
        WINDSHIELD_OPENING_SCALE_Z,
    )
    apply_object_transforms(
        windshield_opening_cutter,
        location=False,
        rotation=True,
        scale=True,
    )
    ...
    if not apply_boolean_difference(
        ...
        raise RuntimeError("Não foi possível abrir o vão do para-brisa.")
...
print("Windshield_Cutter_Base aparado por boolean INTERSECT: sim")
