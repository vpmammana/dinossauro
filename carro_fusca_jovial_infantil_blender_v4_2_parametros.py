...
"""
Carro estilo Fusca jovial — versão 4.2 parametrizável
======================================================
...
- incluir um bloco destacado de parâmetros para editar Front_Hood,
  Headlight_L e Headlight_R sem procurar valores no restante do código.
"""
...
GENERATOR_VERSION = "4.2-fusca-jovial-parametrizavel"
GROUND_SIZE = 24.0
...
# ############################################################################
# #                                                                          #
# #             >>> PARÂMETROS EDITÁVEIS: CAPÔ E FARÓIS <<<                 #
# #                                                                          #
# ############################################################################
#
# SISTEMA DE EIXOS DO MODELO
# --------------------------
# X = PROFUNDIDADE / sentido frente-traseira.
#     Valores maiores de X levam a peça para a FRENTE do carro.
# Y = LARGURA / sentido esquerda-direita.
#     Y positivo = lado L (esquerdo); Y negativo = lado R (direito).
# Z = ALTURA.
#     Valores maiores de Z levam a peça para CIMA.
#
# DIMENSÕES
# ---------
...
#
# Altere SOMENTE os valores deste bloco para reposicionar ou redimensionar
# Front_Hood, Headlight_L e Headlight_R. O restante do script usa esses
...
FRONT_HOOD_PROFUNDIDADE = 1.40   # eixo X; antes era raio 0.50
FRONT_HOOD_LARGURA = 1.5        # eixo Y; antes era raio 0.42
FRONT_HOOD_ALTURA = .7         # eixo Z; antes era raio 0.22
...
# O objeto Front_Hood_Blend é recalculado automaticamente a partir das
# dimensões e da posição de Front_Hood, para acompanhar suas alterações.
# As pupilas também acompanham automaticamente o tamanho e a posição de cada
# farol.
# ############################################################################
# #                    FIM DOS PARÂMETROS EDITÁVEIS                           #
# ############################################################################
...
def dimensions_to_radii(profundidade, largura, altura):
    """Converte dimensões totais X/Y/Z para raios usados pela elipsoide."""
    return (
        profundidade * 0.5,
        largura * 0.5,
        altura * 0.5,
...
def validate_editable_parameters():
    """Interrompe a geração com uma mensagem clara se uma dimensão for inválida."""
    dimensions = {
    ...
    invalid = [name for name, value in dimensions.items() if value <= 0.0]
    if invalid:
        raise ValueError(
            "Todas as dimensões devem ser maiores que zero. Corrija: "
            + ", ".join(invalid)
        )


def create_parameterized_headlight(
    name,
    position,
    dimensions,
    collection,
    ...
    """Cria o farol e uma pupila que acompanha automaticamente seu tamanho."""
    x, y, z = position
    profundidade, largura, altura = dimensions

    create_ellipsoid(
        name,
        local_center=(x, y, z),
        radii=dimensions_to_radii(profundidade, largura, altura),
        collection=collection,
    ...
    pupil_diameter = min(profundidade, largura, altura) * 0.28
    pupil_forward_offset = profundidade * 0.32

    create_ellipsoid(
        name.replace("Headlight", "Pupil"),
        local_center=(x + pupil_forward_offset, y, z),
        radii=dimensions_to_radii(
            pupil_diameter,
            pupil_diameter,
            pupil_diameter,
        ),
...
def build_cute_beetle(scene):
    ...
    # Capô dianteiro parametrizável.
    front_hood_position = (
        FRONT_HOOD_X,
        FRONT_HOOD_Y,
        FRONT_HOOD_Z,
    )
    front_hood_dimensions = (
        FRONT_HOOD_PROFUNDIDADE,
        FRONT_HOOD_LARGURA,
        FRONT_HOOD_ALTURA,
    )
    ...
    create_ellipsoid(
        "Front_Hood",
        local_center=front_hood_position,
        radii=dimensions_to_radii(*front_hood_dimensions),
        collection=model_collection,
        material=materials["body"],
        parent=root,
        subdivisions=2,
    ...
    # O blend acompanha automaticamente o Front_Hood.
    front_hood_blend_position = (
        FRONT_HOOD_X - FRONT_HOOD_PROFUNDIDADE * 0.26,
        FRONT_HOOD_Y,
        FRONT_HOOD_Z + FRONT_HOOD_ALTURA * 0.34,
    )
    front_hood_blend_dimensions = (
        FRONT_HOOD_PROFUNDIDADE * 0.68,
        FRONT_HOOD_LARGURA * 1.095,
        FRONT_HOOD_ALTURA * 0.73,
    )

    create_ellipsoid(
        "Front_Hood_Blend",
        local_center=front_hood_blend_position,
        radii=dimensions_to_radii(*front_hood_blend_dimensions),
        collection=model_collection,
        material=materials["body"],
        parent=root,
        subdivisions=1,
    ...
    # Faróis parametrizáveis de forma independente.
    create_parameterized_headlight(
        name="Headlight_L",
        position=(HEADLIGHT_L_X, HEADLIGHT_L_Y, HEADLIGHT_L_Z),
        dimensions=(
            HEADLIGHT_L_PROFUNDIDADE,
            HEADLIGHT_L_LARGURA,
            HEADLIGHT_L_ALTURA,
        ),
        collection=model_collection,
        headlight_material=materials["headlight"],
        pupil_material=materials["black"],
        parent=root,
    )
    create_parameterized_headlight(
        name="Headlight_R",
        position=(HEADLIGHT_R_X, HEADLIGHT_R_Y, HEADLIGHT_R_Z),
        dimensions=(
            HEADLIGHT_R_PROFUNDIDADE,
            HEADLIGHT_R_LARGURA,
            HEADLIGHT_R_ALTURA,
        ),
        collection=model_collection,
...
print(
    "Front_Hood parâmetros — posição: "
    f"({FRONT_HOOD_X:.3f}, {FRONT_HOOD_Y:.3f}, {FRONT_HOOD_Z:.3f}); "
    "dimensões P/L/A: "
    f"{FRONT_HOOD_PROFUNDIDADE:.3f} / "
    f"{FRONT_HOOD_LARGURA:.3f} / {FRONT_HOOD_ALTURA:.3f}"
)
print(
    "Headlight_L parâmetros — posição: "
    f"({HEADLIGHT_L_X:.3f}, {HEADLIGHT_L_Y:.3f}, {HEADLIGHT_L_Z:.3f}); "
    "dimensões P/L/A: "
    f"{HEADLIGHT_L_PROFUNDIDADE:.3f} / "
    f"{HEADLIGHT_L_LARGURA:.3f} / {HEADLIGHT_L_ALTURA:.3f}"
)
print(
    "Headlight_R parâmetros — posição: "
    f"({HEADLIGHT_R_X:.3f}, {HEADLIGHT_R_Y:.3f}, {HEADLIGHT_R_Z:.3f}); "
    "dimensões P/L/A: "
    f"{HEADLIGHT_R_PROFUNDIDADE:.3f} / "
    f"{HEADLIGHT_R_LARGURA:.3f} / {HEADLIGHT_R_ALTURA:.3f}"
)
