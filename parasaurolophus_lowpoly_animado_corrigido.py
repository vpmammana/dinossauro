"""
Parasaurolophus low-poly colorido e animado para Blender — versão corrigida
====================================================

O script cria:
- um Parasaurolophus estilizado de baixa resolução;
...
COLLECTION_NAME = "Parasaurolophus_Python"
GROUND_Z = 0.0
FPS = 24
FRAME_START = 1
FRAME_END = 24
...
def make_segment(
    ...
    """
    Cria um segmento cônico diretamente entre start e end.

    A malha nasce no eixo correto, em vez de criar um cone vertical e depois
    girá-lo. Isso elimina a rotação aparente de 90 graus em Tail_N, Neck,
    dedos e demais segmentos.
    """
    start = Vector(start)
    end = Vector(end)
    vector = end - start
    ...
    if vertices < 3:
        raise ValueError("Um segmento precisa ter pelo menos 3 lados.")

    axis = vector.normalized()

    # Base perpendicular estável. A primeira direção tende a ficar lateral
    # e a segunda acompanha a dimensão vertical para segmentos horizontais.
    helper = Vector((0.0, 0.0, 1.0))
    if abs(axis.dot(helper)) > 0.92:
        helper = Vector((0.0, 1.0, 0.0))

    basis_u = helper.cross(axis).normalized()
    basis_v = axis.cross(basis_u).normalized()

    # Pequena sobreposição automática esconde frestas nas articulações.
    overlap = min(
        length * 0.065,
        max(float(radius_start), float(radius_end)) * 0.32,
    )
    extended_start = start - axis * overlap
    extended_end = end + axis * overlap

    mesh_vertices = []

    for center, radius in (
        (extended_start, float(radius_start)),
        (extended_end, float(radius_end)),
    ):
        ...
        for index in range(vertices):
            ...
            offset = (
                basis_u * (math.cos(angle) * radius)
                + basis_v * (math.sin(angle) * radius)
            )
            mesh_vertices.append(tuple(local_center + offset))

    start_center_index = len(mesh_vertices)
    mesh_vertices.append(tuple(extended_start - start))

    end_center_index = len(mesh_vertices)
    mesh_vertices.append(tuple(extended_end - start))
    ...
    # Somente as laterais recebem smooth shading.
    for polygon_index, polygon in enumerate(mesh.polygons):
        polygon.use_smooth = polygon_index < vertices

    # Metadados úteis para inspeção no Blender.
    obj["axis_start"] = list(start)
    obj["axis_end"] = list(end)
    obj["axis_length"] = length
...
remove_collection_if_exists("Saurolophus_Python")
...
claw_mat = make_plain_material(
    "MAT_Claws",
    color=(0.065, 0.050, 0.028),
    roughness=0.62,
)
...
make_ellipsoid(
    "TailBase",
    location=(-1.30, 0.0, 2.00),
    scale=(0.82, 0.63, 0.60),
    material=skin,
    collection=collection,
    parent=root,
)
...
# A frente do animal é +X. A crista segue para -X, isto é, para trás.
crest_points = [
    (2.70, 0.0, 3.02),
    (2.28, 0.0, 3.34),
    (1.65, 0.0, 3.56),
...
crest_radii = [0.22, 0.18, 0.11, 0.035]

make_ellipsoid(
    "CrestBase",
    location=(2.64, 0.0, 3.03),
    scale=(0.34, 0.24, 0.22),
    material=crest_mat,
    collection=collection,
    parent=head_ctrl,
    subdivisions=2,
...
for index in range(len(crest_points) - 1):
    make_segment(
        f"Crest_{index + 1:02d}",
        start=crest_points[index],
        end=crest_points[index + 1],
        radius_start=crest_radii[index],
        radius_end=crest_radii[index + 1],
        material=crest_mat,
        collection=collection,
        parent=head_ctrl,
        vertices=9,
    ...
    if index > 0:
        make_joint(
            f"CrestJoint_{index:02d}",
            location=crest_points[index],
            radius=crest_radii[index] * 1.04,
            material=crest_mat,
            collection=collection,
            parent=head_ctrl,
        )
...
# Três dedos baixos, voltados para a frente e levemente para baixo.
    for toe_index, y_offset in enumerate((-0.11, 0.0, 0.11), start=1):
        toe_start = (
            1.15,
            (0.64 + y_offset) * y_sign,
            0.13,
        )
        toe_end = (
            1.67,
            (0.64 + y_offset * 1.30) * y_sign,
            0.09,
        )
        claw_end = (
            1.88,
            (0.64 + y_offset * 1.42) * y_sign,
            0.055,
        )
...
walk_poses = {
    # Contato: esquerda à frente, direita atrás.
    # Em cada perna, hip + knee + ankle = 0, mantendo o pé horizontal.
    1: {
        "root_z": 0.00,
        "hip_l": -13.250,
        "hip_r": 9.500,
    ...
    # Passagem: perna direita elevada; esquerda sob o quadril.
    7: {
        "root_z": 0.00,
        "hip_l": 0.000,
        "hip_r": 9.750,
    ...
    # Contato oposto: esquerda atrás, direita à frente.
    13: {
        "root_z": 0.00,
        "hip_l": 9.500,
        "hip_r": -13.250,
    ...
    # Passagem: perna esquerda elevada; direita sob o quadril.
    19: {
        "root_z": 0.00,
        "hip_l": 9.750,
        "hip_r": 0.000,
...
# Validação da animação no espaço do mundo: procura qualquer parte do pé
# abaixo de Preview_Ground em todos os quadros visíveis.
contact_objects = [
    obj
    for obj in collection.all_objects
    if (
        obj.name.startswith("Foot.")
        or obj.name.startswith("Toe_")
        or obj.name.startswith("Claw_")
        or obj.name.startswith("AnkleJoint.")
    )
]

minimum_foot_z = float("inf")
minimum_foot_frame = FRAME_START
minimum_foot_object = None
...
for frame in range(FRAME_START, FRAME_END + 1):
    ...
    for obj in contact_objects:
        ...
        if object_min_z < minimum_foot_z:
            minimum_foot_z = object_min_z
            minimum_foot_frame = frame
            minimum_foot_object = obj.name

ground_clearance = minimum_foot_z - GROUND_Z

if ground_clearance < -0.001:
    print(
        "AVISO: um pé atravessou o chão: "
        f"{minimum_foot_object}, quadro {minimum_foot_frame}, "
        f"penetração={-ground_clearance:.4f}."
    )
else:
    print(
        "Contato com o chão validado: "
        f"folga mínima={ground_clearance:.4f}, "
        f"quadro={minimum_foot_frame}, objeto={minimum_foot_object}."
    )
...
print(
    "Parasaurolophus corrigido criado com sucesso. "
