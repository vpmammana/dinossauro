"""
Carro estilo Fusca jovial — versão 5
====================================

Ajustes desta versão, conforme solicitado:
- front hood maior;
- front hood menos achatado na vertical;
- front hood levemente mais para trás;
- front hood um pouco mais baixo, encaixando melhor na body shell;
- body shell com corte / quebra de parabrisa mais próximo do Fusca,
  com o para-brisa quase vertical;
- paralamas dianteiros ocos;
- faróis no paralama dianteiro, e não no capô;
- faróis posicionados na parte frontal do Front_Fender_*_Base;
- chassi continua oculto externamente;
...
GENERATOR_VERSION = "5.0-fusca-jovial-front-hood-parabrisa-corrigidos"
GROUND_SIZE = 24.0
...
# ============================================================================
# GEOMETRIA MAIS ORGÂNICA DO FUSCA
# ============================================================================

def create_loft_shell(name, stations, section_points, collection, material,
                      parent=None, local_location=(0.0, 0.0, 0.0)):
    """
    Cria uma casca orgânica loftada.

    Cada station = (x, half_width, z_bottom, z_top, pinch_bottom, fullness_top)

    Nesta versão, as estações próximas ao para-brisa foram alteradas para
    sugerir um corte / quebra quase vertical típico do Fusca.
    """
    vertices = []
    faces = []
...
def create_hollow_fender_arch(
    ...
    material,
    parent=None,
    angle_start_deg=205.0,
    angle_end_deg=-25.0,
    arc_segments=16,
):
    """
    Cria um para-lama oco em forma de arco no plano XZ e espessura no eixo Y.
    Ideal para os paralamas dianteiros do Fusca.
    """
    outer_y = y_mid + side_sign * depth * 0.5
    inner_y = y_mid - side_sign * depth * 0.5

    angles = [
        math.radians(angle_start_deg + (angle_end_deg - angle_start_deg) * i / arc_segments)
        for i in range(arc_segments + 1)
    ]

    def ring_vertices(y_value):
        pts = []
        # arco externo
        for ang in angles:
            x = wheel_center_x + math.cos(ang) * outer_radius
            z = wheel_center_z + math.sin(ang) * outer_radius
            pts.append((x, y_value, z))
        # arco interno, invertido
        for ang in reversed(angles):
            x = wheel_center_x + math.cos(ang) * inner_radius
            z = wheel_center_z + math.sin(ang) * inner_radius
            pts.append((x, y_value, z))
        return pts

    ring_a = ring_vertices(inner_y)
    ring_b = ring_vertices(outer_y)
    ring_len = len(ring_a)

    vertices = ring_a + ring_b
    faces = []

    def vidx(ring_index, idx):
        return ring_index * ring_len + idx

    for i in range(ring_len):
        j = (i + 1) % ring_len
        faces.append((vidx(0, i), vidx(0, j), vidx(1, j), vidx(1, i)))
...
def create_fender_cluster(prefix, center, x_rad, y_rad, z_rad,
    ...
    create_ellipsoid(
        f"{prefix}_Cap",
        local_center=(x, y * 0.95, z + z_rad * 0.28),
        radii=(x_rad * 0.76, y_rad * 0.76, z_rad * 0.42),
        collection=collection,
        material=material,
        parent=parent,
        subdivisions=1,
...
def build_cute_beetle(scene):
    ...
    # ----------------------------------------------------------------------
    # Corpo principal com corte mais característico de para-brisa de Fusca
    # ----------------------------------------------------------------------
    body_stations = [
        # x,   half_width, z_bottom, z_top, pinch_bottom, fullness_top
        ( 1.42, 0.14, 0.54, 0.80, 0.34, 0.04),
        ( 1.26, 0.26, 0.49, 0.90, 0.30, 0.04),
        ( 1.06, 0.40, 0.45, 1.00, 0.26, 0.05),
        ( 0.88, 0.50, 0.43, 1.06, 0.22, 0.05),
        # região da base do para-brisa: sobe bastante e rápido
        ( 0.72, 0.58, 0.42, 1.18, 0.18, 0.06),
        ( 0.58, 0.63, 0.42, 1.36, 0.15, 0.07),
        # topo do parabrisa um pouco mais para trás
        ( 0.44, 0.66, 0.42, 1.52, 0.12, 0.09),
        ( 0.10, 0.68, 0.42, 1.60, 0.11, 0.10),
        (-0.28, 0.65, 0.43, 1.52, 0.14, 0.08),
        (-0.66, 0.58, 0.45, 1.30, 0.18, 0.06),
        (-0.98, 0.50, 0.47, 1.12, 0.22, 0.05),
        (-1.24, 0.38, 0.50, 0.96, 0.26, 0.04),
        (-1.46, 0.18, 0.55, 0.82, 0.32, 0.03),
    ]
    create_loft_shell(
        name="Body_Shell",
    ...
    # Front hood maior, menos achatado, mais para trás e um pouco mais baixo
    create_ellipsoid(
        "Front_Hood",
        local_center=(0.86, 0.0, 0.84),
        radii=(0.68, 0.50, 0.31),
        collection=model_collection,
        material=materials["body"],
        parent=root,
        subdivisions=2,
    ...
    create_ellipsoid(
        "Front_Hood_Blend",
        local_center=(0.60, 0.0, 1.00),
        radii=(0.42, 0.48, 0.20),
        collection=model_collection,
        material=materials["body"],
        parent=root,
        subdivisions=1,
    ...
    create_box(
        "RunningBoard_L",
        local_center=(0.00, 0.67, 0.56),
        size_xyz=(1.96, 0.18, 0.06),
        collection=model_collection,
    ...
    create_box(
        "Windshield",
        local_center=(0.52, 0.0, 1.28),
        size_xyz=(0.08, 0.82, 0.46),
        collection=model_collection,
    ...
    create_trapezoid_window(
        "Side_Window_L_Front",
        center=(0.18, 0.58, 1.28),
        size_xyz=(0.54, 0.04, 0.34),
        collection=model_collection,
        material=materials["glass"],
        parent=root,
    )
    ...
    create_trapezoid_window(
        "Side_Window_L_Rear",
        center=(-0.22, 0.58, 1.22),
        size_xyz=(0.32, 0.04, 0.24),
        collection=model_collection,
        material=materials["glass"],
        parent=root,
    )
    ...
    # Paralamas dianteiros OCOS
    create_hollow_fender_arch(
        name="Front_Fender_L_Base",
        wheel_center_x=0.87,
    ...
    create_ellipsoid(
        "Front_Fender_L_Cap",
        local_center=(0.86, 0.74, 0.88),
        radii=(0.38, 0.18, 0.18),
        collection=model_collection,
        material=materials["body"],
        parent=root,
        subdivisions=1,
    ...
    # Paralamas traseiros podem permanecer cheios
    create_fender_cluster(
        prefix="Rear_Fender_L",
        center=(-0.87, 0.73, 0.74),
        x_rad=0.60, y_rad=0.29, z_rad=0.45,
        collection=model_collection, material=materials["body"], parent=root,
    )
    create_fender_cluster(
        prefix="Rear_Fender_R",
        center=(-0.87, -0.73, 0.74),
        x_rad=0.60, y_rad=0.29, z_rad=0.45,
        collection=model_collection, material=materials["body"], parent=root,
    )
    ...
    # ----------------------------------------------------------------------
    # Faróis no paralama dianteiro, na parte frontal do Front_Fender_*_Base
    # ----------------------------------------------------------------------
    for side_name, side_sign in (("L", 1.0), ("R", -1.0)):
        create_ellipsoid(
            f"Headlight_{side_name}",
            local_center=(1.16, 0.79 * side_sign, 0.93),
        ...
        create_ellipsoid(
            f"Pupil_{side_name}",
            local_center=(1.26, 0.79 * side_sign, 0.93),
    ...
    create_box(
        "Front_Bumper",
        local_center=(1.47, 0.0, 0.62),
        size_xyz=(0.10, 0.88, 0.09),
        collection=model_collection,
        material=materials["accent"],
        parent=root,
    )
    ...
    create_box(
        "Smile_Mid",
        local_center=(1.29, 0.0, 0.74),
        size_xyz=(0.05, 0.36, 0.04),
        collection=model_collection,
        material=materials["black"],
        parent=root,
    )
    create_box(
        "Smile_L",
        local_center=(1.26, 0.11, 0.71),
        size_xyz=(0.04, 0.11, 0.04),
        collection=model_collection,
        material=materials["black"],
        parent=root,
    )
...
print("Front hood maior e mais alto: sim")
print("Front hood mais para trás e mais baixo: sim")
print("Body shell com corte de parabrisa: sim")
print("Paralamas dianteiros ocos: sim")
print("Faróis no paralama dianteiro: sim")
