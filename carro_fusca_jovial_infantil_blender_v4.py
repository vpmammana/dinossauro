"""
Carro estilo Fusca jovial — versão 4
====================================

Esta versão tenta respeitar melhor a lógica geométrica clássica do Fusca:
- um volume principal em arco parabólico / semi-elipse;
- uma dianteira arredondada com capô característico;
- traseira em declive suave tipo gota;
- quatro para-lamas proeminentes, como segmentos esféricos;
- estribos lineares tangentes ligando os para-lamas;
- transições suaves e sem "poliedrização" excessiva;
- sem chassi aparente por fora;
...
GENERATOR_VERSION = "4.0-fusca-jovial-organico"
GROUND_SIZE = 24.0
...
def create_loft_shell(name, stations, section_points, collection, material,
    ...
    A seção transversal é aproximadamente elíptica, porém:
    - mais "fechada" embaixo;
    - mais cheia no topo;
    - com leve verticalidade lateral para evitar forma triangular.
    """
    vertices = []
    faces = []
    ...
    for x, half_width, z_bottom, z_top, pinch_bottom, fullness_top in stations:
        ...
        for i in range(section_points):
            ...
            # alarga no topo e contrai na parte de baixo
            ...
            # retifica um pouco as laterais na região média para parecer carroceria
            if abs(c) < 0.35:
                y *= 0.96
...
def create_trapezoid_window(name, center, size_xyz, collection, material,
    ...
    """
    Janela lateral simplificada em forma de trapézio com cantos superiores sugeridos.
    """
    sx, sy, sz = size_xyz
    hx = sx * 0.5
    hy = sy * 0.5 * mirror_y
...
def create_fender_cluster(prefix, center, x_rad, y_rad, z_rad,
    ...
    """
    Para-lama composto por dois volumes sobrepostos,
    buscando sensação de quarto de esfera integrado à carroceria.
    """
    x, y, z = center
...
def build_cute_beetle(scene):
    ...
    # ----------------------------------------------------------------------
    # Corpo principal: arco contínuo, com frente arredondada e traseira em gota
    # ----------------------------------------------------------------------
    body_stations = [
    ...
    # Capô característico do fusca: uma rampa convexa frontal,
    # sobreposta com suavidade ao arco principal
    create_ellipsoid(
        "Front_Hood",
    ...
    # Traseira mais afilada, mas suave
    create_ellipsoid(
        "Rear_Deck",
    ...
    # Base inferior integrada, escondendo qualquer chassi
    create_box(
        "Lower_Body",
    ...
    # ----------------------------------------------------------------------
    # Rodas e para-lamas: segmentos esféricos proeminentes
    # ----------------------------------------------------------------------
    wheel_positions = {
    ...
    create_fender_cluster(
        prefix="Front_Fender_L",
        center=(0.86, 0.73, 0.73),
        x_rad=0.56, y_rad=0.28, z_rad=0.43,
        collection=model_collection, material=materials["body"], parent=root,
    )
    create_fender_cluster(
        prefix="Front_Fender_R",
        center=(0.86, -0.73, 0.73),
        x_rad=0.56, y_rad=0.28, z_rad=0.43,
        collection=model_collection, material=materials["body"], parent=root,
    )
    ...
    # ----------------------------------------------------------------------
    # Faróis totalmente à frente e virados para frente
    # ----------------------------------------------------------------------
    for side_name, side_sign in (("L", 1.0), ("R", -1.0)):
        create_ellipsoid(
            f"Headlight_{side_name}",
            local_center=(1.30, 0.28 * side_sign, 0.98),
        ...
        create_ellipsoid(
            f"Pupil_{side_name}",
            local_center=(1.40, 0.28 * side_sign, 0.98),
    ...
    # ----------------------------------------------------------------------
    # Para-choques e detalhes infantis discretos
    # ----------------------------------------------------------------------
    create_box(
        "Front_Bumper",
        local_center=(1.46, 0.0, 0.62),
...
print("=" * 72)
print("Carro estilo Fusca jovial orgânico criado com sucesso.")
...
print("Arco central contínuo: sim")
print("Capô dianteiro característico: sim")
print("Traseira em declive suave: sim")
print("Paralamas hemisféricos proeminentes: sim")
print("Estribos tangentes: sim")
