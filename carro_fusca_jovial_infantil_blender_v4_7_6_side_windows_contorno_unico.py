"""
Carro estilo Fusca jovial — versão 4.7.6 parametrizada
======================================================

Correção geométrica das janelas laterais:
- elimina os quatro booleans independentes das janelas laterais;
- calcula o contorno do vidro pela interseção entre UM plano lateral reto
  e uma cópia intacta da Body_Shell;
- faz apenas UM boolean por lado, contendo as duas janelas daquele lado;
- a face externa do vidro é exatamente a face externa usada pelo cutter;
- o volume do cutter segue somente para dentro da carroceria;
- o contorno visível do vidro já nasce recortado pela Body_Shell;
- preserva o pilar central entre vidro dianteiro e traseiro;
- evita cortes sobrepostos no capô/teto e remove os dentes laterais;
- o objeto Rear_Window visível continua removido.
"""
...
GENERATOR_VERSION = "4.7.6-fusca-jovial-side-window-single-contour"
GROUND_SIZE = 24.0
...
# ---------------------------------------------------------------------------
BODY_SHELL_SECTION_POINTS = 32

# Número de segmentos criados entre cada par de estações longitudinais.
# Aumentar suaviza a linha de interseção dos vidros com a carroceria.
BODY_SHELL_LONGITUDINAL_SUBDIVISIONS = 4
BODY_SHELL_STATIONS = [
...
# ---------------------------------------------------------------------------
# SIDE WINDOWS — um único plano e um único boolean por lado
# ---------------------------------------------------------------------------
# O plano inclinado é comum às janelas dianteira e traseira.
# A parte superior entra na cabine pelo ângulo abaixo.
SIDE_WINDOW_PLANE_Y = 0.59
SIDE_WINDOW_PLANE_Z = 1.25
SIDE_WINDOW_INWARD_TILT_DEG = 12.0

# Limites longitudinais do conjunto de vidros no eixo X.
# O limite frontal fica atrás do corte do para-brisa, preservando o pilar A.
SIDE_WINDOW_X_MIN = -0.39
SIDE_WINDOW_X_MAX = 0.36

# Pilar central preservado na Body_Shell.
SIDE_WINDOW_B_PILLAR_X = -0.04
SIDE_WINDOW_B_PILLAR_WIDTH = 0.065

# O cutter nasce com a MESMA borda externa do vidro e segue só para dentro.
SIDE_WINDOW_CUT_DEPTH = 0.76

# Pequena tolerância numérica do cutter para fora. Não altera o perfil X/Z.
SIDE_WINDOW_CUT_OUTWARD_EPSILON = 0.002

# Espessura do vidro: toda a espessura segue para dentro da carroceria.
SIDE_WINDOW_GLASS_THICKNESS = 0.035
SIDE_WINDOW_GLASS_OUTSET = 0.000

# Número mínimo de pontos exigido em cada metade do contorno.
SIDE_WINDOW_MIN_POLYGON_POINTS = 3

# ---------------------------------------------------------------------------
# REAR WINDOW — somente recorte; nenhum vidro visível é criado
# ---------------------------------------------------------------------------
REAR_WINDOW_X = -0.58
REAR_WINDOW_Y = 0.00
REAR_WINDOW_Z = 1.20
...
def validate_parameters():
    positive = {
        ...
        "SIDE_WINDOW_CUT_DEPTH": SIDE_WINDOW_CUT_DEPTH,
        "SIDE_WINDOW_CUT_OUTWARD_EPSILON": SIDE_WINDOW_CUT_OUTWARD_EPSILON,
        "SIDE_WINDOW_GLASS_THICKNESS": SIDE_WINDOW_GLASS_THICKNESS,
        "SIDE_WINDOW_B_PILLAR_WIDTH": SIDE_WINDOW_B_PILLAR_WIDTH,
    ...
    if BODY_SHELL_LONGITUDINAL_SUBDIVISIONS < 1:
        raise ValueError(
            "BODY_SHELL_LONGITUDINAL_SUBDIVISIONS deve ser >= 1."
        )

    if SIDE_WINDOW_X_MAX <= SIDE_WINDOW_X_MIN:
        raise ValueError("SIDE_WINDOW_X_MAX deve ser maior que SIDE_WINDOW_X_MIN.")

    pillar_left = SIDE_WINDOW_B_PILLAR_X - SIDE_WINDOW_B_PILLAR_WIDTH * 0.5
    pillar_right = SIDE_WINDOW_B_PILLAR_X + SIDE_WINDOW_B_PILLAR_WIDTH * 0.5
    if pillar_left <= SIDE_WINDOW_X_MIN or pillar_right >= SIDE_WINDOW_X_MAX:
        raise ValueError("O pilar B deve ficar dentro dos limites das janelas laterais.")
...
def densify_loft_stations(stations, subdivisions):
    """
    Interpola as estações longitudinalmente antes de criar a malha.
    Isso reduz facetas e o serrilhado visível nas linhas booleanas.
    """
    ...
    for index in range(len(stations) - 1):
        ...
        for step in range(subdivisions):
            ...
            dense.append(
                tuple(
                    (1.0 - factor) * float(start[channel])
                    + factor * float(end[channel])
...
def create_loft_shell(name, stations, section_points, collection, material,
                      parent=None, local_location=(0.0, 0.0, 0.0),
                      longitudinal_subdivisions=1):
    vertices = []
    faces = []

    dense_stations = densify_loft_stations(
        stations,
        longitudinal_subdivisions,
    )

    for x, half_width, z_bottom, z_top, pinch_bottom, fullness_top in dense_stations:
    ...
    ring = section_points
    station_count = len(dense_stations)
    ...
    fx, _, fzb, fzt, _, _ = dense_stations[0]
    ...
    rx, _, rzb, rzt, _, _ = dense_stations[-1]
...
def clean_polygon_2d(points, tolerance=1.0e-8):
    cleaned = []
    for point in points:
        ...
        previous = cleaned[-1]
        if (
            abs(current[0] - previous[0]) > tolerance
            or abs(current[1] - previous[1]) > tolerance
        ):
            cleaned.append(current)
...
def clip_polygon_at_u(points, boundary, keep_greater, tolerance=1.0e-9):
    """Recorta um polígono 2D pela reta vertical u=boundary."""
    points = clean_polygon_2d(points)
    if len(points) < 3:
        return []

    def inside(point):
        if keep_greater:
            return point[0] >= boundary - tolerance
        return point[0] <= boundary + tolerance
    ...
    return clean_polygon_2d(result)


def clip_polygon_u_range(points, u_min, u_max):
    clipped = clip_polygon_at_u(points, u_min, keep_greater=True)
    clipped = clip_polygon_at_u(clipped, u_max, keep_greater=False)
    return clean_polygon_2d(clipped)


def side_window_plane_basis(side_sign):
    rotation = Euler(
        (
            math.radians(float(side_sign) * SIDE_WINDOW_INWARD_TILT_DEG),
            0.0,
            0.0,
        ),
        "XYZ",
    ...
    plane_center = Vector(
        (
            0.0,
            float(side_sign) * SIDE_WINDOW_PLANE_Y,
            SIDE_WINDOW_PLANE_Z,
        )
    )
    axis_u = (matrix @ Vector((1.0, 0.0, 0.0))).normalized()
    axis_v = (matrix @ Vector((0.0, 0.0, 1.0))).normalized()
    outward = (
        matrix @ Vector((0.0, float(side_sign), 0.0))
    ).normalized()
    return plane_center, axis_u, axis_v, outward


def orient_polygon_for_outward(points, plane_center, axis_u, axis_v, outward):
    points = clean_polygon_2d(points)
    if len(points) < 3:
        return points

    p0 = plane_center + axis_u * points[0][0] + axis_v * points[0][1]
    p1 = plane_center + axis_u * points[1][0] + axis_v * points[1][1]
    p2 = plane_center + axis_u * points[2][0] + axis_v * points[2][1]
...
def append_inward_prism_geometry(
    vertices,
    faces,
    polygon_uv,
    plane_center,
    ...
    """
    Adiciona um prisma cujo plano externo é o contorno do vidro.

    A face externa fica em ``outward_offset`` e todo o volume restante
    segue exclusivamente para dentro da carroceria.
    """
    polygon_uv = orient_polygon_for_outward(
        polygon_uv,
        plane_center,
        axis_u,
        axis_v,
    ...
    if len(polygon_uv) < 3:
        raise ValueError("O contorno lateral precisa ter pelo menos três pontos.")
    ...
    inner_points = [
        point - outward * (inward_depth + outward_offset)
        for point in outer_points
    ]

    vertices.extend(tuple(point) for point in outer_points)
    vertices.extend(tuple(point) for point in inner_points)

    count = len(polygon_uv)
    outer_face = tuple(start_index + index for index in range(count))
    inner_face = tuple(
...
def create_prism_from_uv_polygon(
    name,
    polygon_uv,
    plane_center,
    axis_u,
...
def body_shell_side_window_polygon(body_shell_reference, root, side_sign):
    """
    Calcula o contorno real do vidro no plano lateral.

    A borda superior e a inferior vêm da interseção do plano reto do vidro
    com a Body_Shell intacta. Só os limites dianteiro/traseiro e o pilar B
    são cortes lineares artificiais.
    """
    plane_center, axis_u, axis_v, outward = side_window_plane_basis(side_sign)
    points = mesh_plane_intersection_points(
        body_shell_reference,
        plane_center,
        outward,
        root_obj=root,
        tolerance=1.0e-6,
    ...
    if len(points) < 6:
        raise RuntimeError(
            f"Poucos pontos na interseção lateral do lado {side_sign:+.0f}: "
            f"{len(points)}. Ajuste SIDE_WINDOW_PLANE_Y/Z."
        )
    ...
    if len(polygon) < 4:
        raise RuntimeError(
            "O plano lateral não gerou uma região de janela válida. "
            "Ajuste SIDE_WINDOW_X_MIN/MAX ou SIDE_WINDOW_PLANE_Y/Z."
        )

    return polygon, plane_center, axis_u, axis_v, outward


def create_side_windows_for_side(
    body_shell,
    body_shell_reference,
    root,
    side_sign,
    ...
    """Um único boolean por lado; dois vidros separados pelo pilar original."""
    polygon, plane_center, axis_u, axis_v, outward = (
        body_shell_side_window_polygon(
            body_shell_reference,
            root,
            side_sign,
        )
    ...
    rear_polygon = clip_polygon_at_u(
        polygon,
        pillar_left,
        keep_greater=False,
    )
    front_polygon = clip_polygon_at_u(
        polygon,
        pillar_right,
        keep_greater=True,
    )

    minimum = max(3, int(SIDE_WINDOW_MIN_POLYGON_POINTS))
    if len(front_polygon) < minimum or len(rear_polygon) < minimum:
        raise RuntimeError(
            "O pilar B deixou um dos vidros laterais sem contorno suficiente."
        )

    cutter_vertices = []
    cutter_faces = []
    for component in (front_polygon, rear_polygon):
        append_inward_prism_geometry(
            vertices=cutter_vertices,
            faces=cutter_faces,
            polygon_uv=component,
            plane_center=plane_center,
            axis_u=axis_u,
            axis_v=axis_v,
            outward=outward,
            inward_depth=SIDE_WINDOW_CUT_DEPTH,
            outward_offset=SIDE_WINDOW_CUT_OUTWARD_EPSILON,
        )

    side_suffix = "L" if side_sign > 0 else "R"
    cutter = create_mesh_object(
        name=f"CUT_Side_Windows_{side_suffix}",
        vertices=cutter_vertices,
        faces=cutter_faces,
        collection=collection,
        material=None,
    ...
    if not apply_boolean_difference(
        ...
        raise RuntimeError(
            f"Não foi possível aplicar o único recorte lateral do lado {side_suffix}."
        )

    front_glass = create_prism_from_uv_polygon(
        name=f"Side_Window_{side_suffix}_Front",
        polygon_uv=front_polygon,
        plane_center=plane_center,
        axis_u=axis_u,
        axis_v=axis_v,
        outward=outward,
        inward_depth=SIDE_WINDOW_GLASS_THICKNESS,
        outward_offset=SIDE_WINDOW_GLASS_OUTSET,
        collection=collection,
        material=glass_material,
        parent=root,
    )
    rear_glass = create_prism_from_uv_polygon(
        name=f"Side_Window_{side_suffix}_Rear",
        polygon_uv=rear_polygon,
        plane_center=plane_center,
        axis_u=axis_u,
        axis_v=axis_v,
        outward=outward,
    ...
    front_glass["outer_surface_defines_body_cut"] = True
    rear_glass["outer_surface_defines_body_cut"] = True
    return front_glass, rear_glass
...
def build_cute_beetle(scene):
    ...
    body_shell = create_loft_shell(
        ...
        parent=root,
        local_location=(0.0, 0.0, 0.0),
        longitudinal_subdivisions=BODY_SHELL_LONGITUDINAL_SUBDIVISIONS,
    )
    body_shell.scale = (
        BODY_SHELL_ESCALA_PROFUNDIDADE,
        BODY_SHELL_ESCALA_LARGURA,
    ...
    # Referência intacta: os contornos laterais são calculados antes de
    # qualquer abertura, evitando que um boolean anterior gere dentes.
    body_shell_window_reference = duplicate_mesh_object(
        body_shell,
        "TMP_BodyShell_SideWindowReference",
        model_collection,
        parent=root,
    )
    body_shell_window_reference.hide_viewport = True
    body_shell_window_reference.hide_render = True
    ...
    # ----------------------------------------------------------------------
    # SIDE WINDOWS: contorno da Body_Shell intacta + um boolean por lado
    # ----------------------------------------------------------------------
    side_window_l_front, side_window_l_rear = create_side_windows_for_side(
        body_shell=body_shell,
        body_shell_reference=body_shell_window_reference,
        root=root,
        side_sign=1.0,
        collection=model_collection,
    ...
    side_window_r_front, side_window_r_rear = create_side_windows_for_side(
        body_shell=body_shell,
        body_shell_reference=body_shell_window_reference,
        root=root,
        side_sign=-1.0,
    ...
    # A referência não participa do render nem dos booleans seguintes.
    delete_object(body_shell_window_reference)

    # Janela traseira: somente o vão permanece. Nenhum Rear_Window é criado.
    ...
    if not apply_boolean_difference(
        ...
        raise RuntimeError(
            "Não foi possível abrir o recorte da janela traseira."
        )
...
print("Objeto Rear_Window visível presente: não")
print("Recorte traseiro preservado; cutter temporário removido: sim")
