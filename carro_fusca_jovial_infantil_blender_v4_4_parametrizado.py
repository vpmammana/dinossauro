"""
Carro estilo Fusca jovial — versão 4.4 parametrizada
====================================================

Resumo desta versão:
- usa os parâmetros fornecidos pelo usuário para front hood, headlights e
  front fenders;
- adiciona parametrização completa para rear fenders;
- body shell permanece cortada a partir da região do windshield;
- windshield cobre toda a extensão lateral da abertura frontal remanescente;
- side windows e rear window geram corte na body shell via boolean;
- rear window fica inclinada de acordo com a descida traseira da body shell;
- carro pintado de vermelho;
- vidros em azul transparente;
- script idempotente (limpa a cena antes de reconstruir).
"""
...
GENERATOR_VERSION = "4.4-fusca-jovial-parametrizado-red-glasscut"
GROUND_SIZE = 24.0
...
#
# Observação:
#   As elipsoides são criadas por raios. Internamente o script divide por 2.
# ############################################################################
...
WINDSHIELD_X = 0.47
WINDSHIELD_Z = 1.30
WINDSHIELD_WIDTH = 1.34
WINDSHIELD_HEIGHT = 0.58
WINDSHIELD_THICKNESS = 0.05
WINDSHIELD_TILT_DEG = -10.0
WINDSHIELD_BOTTOM_INSET = 0.22   # recuo lateral inferior para seguir o capô
...
SIDE_WINDOW_FRONT_THICKNESS = 0.035
...
SIDE_WINDOW_REAR_THICKNESS = 0.035
...
REAR_WINDOW_THICKNESS = 0.05
REAR_WINDOW_TILT_DEG = 33.0

# Boolean cutter oversize
CUTTER_DEPTH_EXTRA = 0.30
...
def validate_parameters():
    positive_params = {
        ...
        "WINDSHIELD_WIDTH": WINDSHIELD_WIDTH,
...
def create_windshield_panel(name, center, width, height, thickness,
                            bottom_inset, collection, material, parent=None,
                            rotation=(0.0, 0.0, 0.0)):
    hy_top = width * 0.5
    hy_bottom = max(0.05, hy_top - bottom_inset)
    hz = height * 0.5
    hx = thickness * 0.5
...
def apply_boolean_difference(target_obj, cutter_obj):
    ctx_view_layer = bpy.context.view_layer
    ctx_view_layer.objects.active = target_obj
    for obj in ctx_view_layer.objects:
        obj.select_set(False)
    target_obj.select_set(True)

    modifier = target_obj.modifiers.new(
        name=f"BOOL_{cutter_obj.name}", type="BOOLEAN"
    )
...
def build_cute_beetle(scene):
    ...
    # BODY SHELL: já nasce cortada a partir do windshield
    body_stations = [
        ( WINDSHIELD_CUT_X,      0.67, 0.42, 1.52, 0.15, 0.10),
        ( WINDSHIELD_CUT_X-0.20, 0.70, 0.42, 1.60, 0.12, 0.10),
        ( WINDSHIELD_CUT_X-0.54, 0.66, 0.43, 1.50, 0.14, 0.08),
        ( WINDSHIELD_CUT_X-0.90, 0.58, 0.45, 1.30, 0.18, 0.06),
        ( WINDSHIELD_CUT_X-1.22, 0.50, 0.47, 1.12, 0.22, 0.05),
        ( WINDSHIELD_CUT_X-1.48, 0.36, 0.50, 0.96, 0.26, 0.04),
        ( WINDSHIELD_CUT_X-1.72, 0.18, 0.55, 0.82, 0.32, 0.03),
    ]
    ...
    # VIDROS E CORTES
    # ----------------------------------------------------------------------
    # Windshield: painel com recorte inferior acompanhando a largura do capô
    windshield = create_windshield_panel(
        "Windshield",
        center=(WINDSHIELD_X, 0.0, WINDSHIELD_Z),
        width=WINDSHIELD_WIDTH,
        height=WINDSHIELD_HEIGHT,
        thickness=WINDSHIELD_THICKNESS,
        bottom_inset=WINDSHIELD_BOTTOM_INSET,
        collection=model_collection,
        material=materials["glass"],
        parent=root,
        rotation=(0.0, math.radians(WINDSHIELD_TILT_DEG), 0.0),
    )

    # Side windows front
    side_window_l_front = create_side_window_profile(
        "Side_Window_L_Front",
        center=(SIDE_WINDOW_FRONT_X, 0.59, SIDE_WINDOW_FRONT_Z),
        size_xyz=(
    ...
    side_window_l_rear = create_side_window_profile(
        "Side_Window_L_Rear",
        center=(SIDE_WINDOW_REAR_X, 0.59, SIDE_WINDOW_REAR_Z),
        size_xyz=(
    ...
    # Rear window inclinada conforme queda traseira
    rear_window = create_box(
        "Rear_Window",
        local_center=(REAR_WINDOW_X, 0.0, REAR_WINDOW_Z),
        size_xyz=(
            REAR_WINDOW_THICKNESS,
            REAR_WINDOW_WIDTH,
            REAR_WINDOW_HEIGHT,
        ),
        collection=model_collection,
        ...
        parent=root,
        smooth=False,
        rotation=(0.0, math.radians(REAR_WINDOW_TILT_DEG), 0.0),
    )

    # Cutters para side windows e rear window
    cutters = []
    ...
    for obj, name in (
        ...
        cutter = create_boolean_cutter_from_object(obj, name, model_collection, parent=root)
        # engrossa ligeiramente o cutter para garantir o corte
        cutter.scale.y *= 1.0 + CUTTER_DEPTH_EXTRA
        cutter.scale.x *= 1.2
        cutter.scale.z *= 1.12
    ...
    create_ellipsoid(
        "Rear_Fender_L_Base",
        local_center=(
            REAR_FENDER_L_BASE_X,
            REAR_FENDER_L_BASE_Y,
            REAR_FENDER_L_BASE_Z,
        ),
        radii=(
    ...
    create_ellipsoid(
        "Rear_Fender_R_Base",
        local_center=(
            REAR_FENDER_R_BASE_X,
            REAR_FENDER_R_BASE_Y,
            REAR_FENDER_R_BASE_Z,
        ),
        radii=(
    ...
    # ----------------------------------------------------------------------
    # Para-choques e pequenos detalhes
    # ----------------------------------------------------------------------
    create_arc_bumper(
        "Front_Bumper",
...
print("Vidros azuis transparentes: sim")
print("Body shell cortada a partir do windshield: sim")
print("Side windows e rear window com corte na body shell: sim")
print("Rear window inclinada conforme a traseira: sim")
print("Rear fenders parametrizados em 6 parâmetros cada: sim")
